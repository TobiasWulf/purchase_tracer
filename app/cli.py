...
def register(app):
    ...
    def init(lang: str):
        ...
        :param lang: Language identifier after ISO639 e.g. 'de' for german or
                     'nl' for dutch.
        :type lang: str.
        :raises: RunTimeError.
        """
    ...
    def remove(lang: str):
        ...
        [
            os.remove(f) for f in gl.glob(op.join(path, '**'), recursive=True)
            if op.isfile(f)
        ]
        [
            os.removedirs(d) for d in gl.glob(
                op.join(path, '**'),
                recursive=True
            )[::-1] if op.isdir(d)
        ]
